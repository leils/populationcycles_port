...
void run(int id){
  ...
  */
      ...
      for (int i=0; i < 8; i++) {                                                    // for each neighbor cell
...
void setType(int id, color newType) {                                                      // if neighbor cell can be eaten,
...
void render(int id) {                                                                       // draw the actual dots to represent lifeforms in each cell

  if(!dead(cells[id][0])){                                                                    // if it's not dead
    sim.fill(cells[id][2]);                                                               // make it the correct color
    sim.stroke(cells[id][2]);
    
    if (cellShape == 0) {                                                              // then draw a cirlce of cellShape is set to 0
      sim.ellipse(0.8*s+(cells[id][0] % rowSize)*s, 0.5*s+floor(cells[id][0]/rowSize)*s, s, s);
    }
    else if (cellShape == 1) {
                                                                 // or draw a square if cellShape is set to 1
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s), int(0.3*s+floor(cells[id][0]/rowSize)*s), cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s)+1, int(0.3*s+floor(cells[id][0]/rowSize)*s), cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s), int(0.3*s+floor(cells[id][0]/rowSize)*s)+1, cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s)+1, int(0.3*s+floor(cells[id][0]/rowSize)*s)+1, cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s), int(0.3*s+floor(cells[id][0]/rowSize)*s)+2, cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s)+1, int(0.3*s+floor(cells[id][0]/rowSize)*s)+2, cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s)+2, int(0.3*s+floor(cells[id][0]/rowSize)*s)+2, cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s)+2, int(0.3*s+floor(cells[id][0]/rowSize)*s)+1, cells[id][2]);
      sim.set(int(0.5*s+(cells[id][0] % rowSize)*s)+2, int(0.3*s+floor(cells[id][0]/rowSize)*s), cells[id][2]);
    }
  }
...
boolean dead(int id) {                                                                 // check to see if cell is alive or not
...
void paintCells(int paintedCellID){
  
  fill(paintType);
  stroke(paintType);
  
  int[] paintedCells = findNeighbors(paintedCellID);
  int[] paintedNeighbors;
